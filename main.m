...
k1 = 1;             % CepR*-CepI activation coefficient  http://2018.igem.org/Team:Tsinghua/Model?1
k2 = 1;             % CciR*-CepI/R repression coefficient
k3 = 1;             % CepR* - CepR repression coefficient
k4 = 1;
k5 = 1;
...
%% set the beta2 transcription to 0 %%
% this is done to compare the core vs. the complete model. Setting the
% beta2 to zero should lead to the same results when using M1 and M2.
% beta2 = 0;
...
figure(5)
subplot(1,3,1)
plot(T,Ys(:,1), 'LineWidth', 2);
title('CEPI')
hold on
subplot(1,3,2)
plot(T,Ys(:,4), 'LineWidth', 2);
title('CEPR')
hold on
subplot(1,3,3)
plot(T,Ys(:,5), 'LineWidth', 2);
title('CEPRstar')

sgtitle('Concentration of the main species in the core model')


figure(6)
subplot(2,3,1)
plot(T2,Y2s(:,1));
title('CEPI (complete model)', 'LineWidth', 2)
hold on
subplot(2,3,2)
plot(T2,Y2s(:,4));
title('CEPR (complete model)', 'LineWidth', 2)
hold on
subplot(2,3,3)
plot(T2,Y2s(:,5));
...
sgtitle('Concentration of the main species in the complete model')
...
figure(10)
plot(Ys(:,1)./Ys(end,1), Ys(:,2)./Ys(end,2), 'LineWidth', 2);
hold on  
figure(11)
plot(Y2s(:,1)./Y2s(end,1), Y2s(:,4)./Y2s(end,4), 'LineWidth', 2);
hold on
%compute response time for core (RT_1) and complete (RT_2) models
